...
void Anchor::SendPacket_UpdateTeamState() {
    ...
    if (IS_RANDO) {
        ...
        for (int i = 0; i < RC_MAX; i++) {
            payload["state"]["rando"]["itemLocations"][i] = json::object();
            // payload["state"]["rando"]["itemLocations"][i]["rgID"] = randoContext->GetItemLocation(i)->GetPlacedRandomizerGet();
            payload["state"]["rando"]["itemLocations"][i]["status"] = randoContext->GetItemLocation(i)->GetCheckStatus();
            payload["state"]["rando"]["itemLocations"][i]["skipped"] = randoContext->GetItemLocation(i)->GetIsSkipped();
...
void Anchor::HandlePacket_UpdateTeamState(nlohmann::json payload) {
    ...
    if (IS_RANDO && payload["state"].contains("rando")) {
        ...
        for (int i = 0; i < RC_MAX; i++) {
            ...
            OTRGlobals::Instance->gRandoContext->GetItemLocation(i)->SetCheckStatus(payload["state"]["rando"]["itemLocations"][i]["status"].get<RandomizerCheckStatus>());
            OTRGlobals::Instance->gRandoContext->GetItemLocation(i)->SetIsSkipped(payload["state"]["rando"]["itemLocations"][i]["skipped"].get<bool>());
