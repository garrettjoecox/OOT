...
#define CVAR_TELEPORT_MODE_NAME CVAR_ENHANCEMENT("ExtraTraps.TeleportMode")
...
const char* altTrapTypeCvars[] = {
    CVAR_ENHANCEMENT("ExtraTraps.Ice"),       // ADD_ICE_TRAP
    CVAR_ENHANCEMENT("ExtraTraps.Burn"),      // ADD_BURN_TRAP
    CVAR_ENHANCEMENT("ExtraTraps.Shock"),     // ADD_SHOCK_TRAP
    CVAR_ENHANCEMENT("ExtraTraps.Knockback"), // ADD_KNOCK_TRAP
    CVAR_ENHANCEMENT("ExtraTraps.Speed"),     // ADD_SPEED_TRAP
...
// Advanced Teleport Entrances
static const std::vector<uint32_t> sTeleportTrapEntrances = {
...
static void DoTeleportTrapSimple() {
    int entrance;
...
static void DoTeleportTrapAdvanced() {
    if (sTeleportTrapEntrances.empty()) {
        // Paranoid fallback
        DoTeleportTrapSimple();
        return;
    }

    uint32_t entrance = RandomElement(sTeleportTrapEntrances);

    // Use the same helper as Simple so we get the laugh + proper transition handling
    GameInteractor::RawAction::TeleportPlayer(static_cast<int32_t>(entrance));
}
    ...
    if (eventTimer == 0) {
        switch (roll) {
            ...
            case ADD_AMMO_TRAP:
                ...
                break;
            case ADD_TELEPORT_TRAP: {
                int teleportMode = CVarGetInteger(CVAR_TELEPORT_MODE_NAME, TELEPORT_MODE_SIMPLE);
                if (teleportMode == TELEPORT_MODE_ADVANCED) {
                    DoTeleportTrapAdvanced();
                } else {
                    DoTeleportTrapSimple();
                }
                break;
