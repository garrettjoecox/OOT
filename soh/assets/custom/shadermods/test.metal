...
@end
    ...
    // --- Breath of the Wild Style Fake Toon Shading ---
    // 1. Get base luminance
    float lum = dot(texel.xyz, float3(0.299, 0.587, 0.114));

    // 2. Define soft ramps for the "terminator" lines. 
    // smoothstep gives us that slightly soft transition BotW is known for.
    float shadowRamp = smoothstep(0.35, 0.45, lum);
    float highlightRamp = smoothstep(0.75, 0.85, lum);

    // 3. Define the stylized color tiers
    // Shadows drop in brightness and get a cooler, bluish tint
    float3 shadowColor = texel.xyz * float3(0.65, 0.65, 0.85); 
    
    // Midtones are just the base texture
    float3 midColor = texel.xyz; 
    
    // Highlights get a warm, glowing boost
    float3 highlightColor = texel.xyz + float3(0.15, 0.10, 0.0); 

    // 4. Blend the tiers together based on our ramps
    float3 botwColor = mix(shadowColor, midColor, shadowRamp);
    botwColor = mix(botwColor, highlightColor, highlightRamp);

    // 5. BotW is highly vibrant, so let's push the saturation up a bit (1.25x)
    float newLum = dot(botwColor, float3(0.299, 0.587, 0.114));
    texel.xyz = mix(float3(newLum), botwColor, 1.25);
